...
export function unitCategoriesPage({
    storeUrl,
    updateUrlTemplate,
    deleteUrlTemplate,
    ...
    return composeDataTable(
        ...
        },
        {
                ...
                if (!next && deactivateCheckUrlTemplate) {
                    ...
                    if (count > 0) {
                        this.$store.confirm.show({
                            ...
                            message:      `This category is used by ${count} unit${count > 1 ? 's' : ''}. Are you sure you want to deactivate it?`,
                            intent:       'warning',
            ...
            confirmDeleteRow(id, message = null) {
                const row  = this.rowsById[id];
                const name = row?.name ?? 'this category';
                this.$store.confirm.show({
                    title:        `Delete "${name}"?`,
                    message:      message,
                    intent:       'danger',
            ...
            _applyServerResponse(data) {
                ...
                if (data.list_html) {
                    const list = this._rootEl.querySelector('.unit-cat-list');
                    if (list) {
